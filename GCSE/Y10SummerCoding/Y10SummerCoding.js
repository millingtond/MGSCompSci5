'use strict';
// GCSE Python Summer Project - Main JavaScript File
...
(function() {
    ...
    // --- UI GENERATION AND INITIALIZATION ---
    ...
    function switchTask(taskId) {
        try {
            ...
            const editorDiv = safeCreateElement('div');
            editorDiv.id = 'editor';
            ...
            runBtn.innerHTML = '▶ Run <small style="font-size: 0.75rem; opacity: 0.7; margin-left: 0.3rem;">(Ctrl+Enter)</small>';
    ...
    function initializeEditor(starterCode) {
        ...
        if (appState.editor) {
            if (typeof appState.editor.destroy === 'function') {
                appState.editor.destroy();
            }
            appState.editor = null;
        }
        
        // Check if we can use a fallback textarea if Ace fails
        if (typeof ace === 'undefined') {
            console.error('Ace Editor not loaded, using fallback textarea');
            showToast('Using basic editor (advanced features disabled)', 'warning');
            editorDiv.innerHTML = '<textarea id="editor-fallback" style="width: 100%; height: 100%; background: #272822; color: #f8f8f2; font-family: monospace; font-size: 14px; padding: 10px; border: none; resize: none;"></textarea>';
            const textarea = document.getElementById('editor-fallback');
            textarea.value = appState.userProgress[appState.currentTaskId]?.code || starterCode;
            
            // Create a simple fallback editor object
            appState.editor = {
                getValue: () => textarea.value,
                setValue: (val, cursorPos) => { textarea.value = val; },
                destroy: () => {},
            ...
            textarea.addEventListener('input', () => {
                ...
                appState.userProgress[appState.currentTaskId].code = textarea.value;
                saveProgress();
            });
        ...
        // Disable paste
        appState.editor.on('paste', function(e) {
            if (e.text !== undefined) {
                e.text = '';
            }
            showToast('Paste is disabled. Please type your code.', 'warning');
        ...
        appState.editor.setValue(savedCode, -1); // -1 moves cursor to start
    ...
    function resetCode() {
        if (confirm('Are you sure you want to reset your code for this task? This cannot be undone.')) {
            if (typeof appState.editor.setValue === 'function') {
                appState.editor.setValue(TASKS[appState.currentTaskId].starterCode, -1);
            } else {
    ...
    async function testConnectivity() {
        try {
            ...
            const testUrls = [
                ...
                'https://cdn.jsdelivr.net/npm/skulpt@1.2.0/package.json',
                'https://unpkg.com/'
            ];
            ...
            for (const url of testUrls) {
                try {
                    ...
                    console.log(`Connectivity test passed: ${url}`);
                } catch (e) {
                    console.warn(`Connectivity test failed: ${url}`);
                }
            }
        } catch (e) {
    ...
    async function loadSkulptWithFallbacks() {
        ...
        if (typeof Sk !== 'undefined') {
            window.skulptLoadAttempts.push('Already loaded');
            return true;
        }
        ...
        for (const source of skulptSources) {
            ...
            try {
                ...
                if (typeof Sk !== 'undefined') {
                    ...
                    window.skulptLoadAttempts.push(`${source.name}: Success`);
                    return true;
                } else {
                    window.skulptLoadAttempts.push(`${source.name}: Loaded but Sk undefined`);
                }
            } catch (e) {
                console.warn(`Failed to load from ${source.name}:`, e);
                window.skulptLoadAttempts.push(`${source.name}: ${e.message}`);
            }
        }
    ...
    function loadScript(src) {
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                script.remove();
                reject(new Error(`Timeout loading ${src} after ${timeoutDuration/1000} seconds`));
            }, timeoutDuration);
        ...
        try {
            ...
            if (!skulptLoaded) {
                ...
                const diagnosticInfo = [
                    ...
                    `Ace Editor loaded: ${typeof ace !== 'undefined'}`,
                    `Skulpt loaded: ${typeof Sk !== 'undefined'}`,
                    `Load attempts: ${window.skulptLoadAttempts.join(', ')}`,
                ...
                document.getElementById('loading-message').innerHTML = `
                    ...
                    <details style="margin: 1rem 0; text-align: left; max-width: 600px; margin: 0 auto;">
                        <summary style="cursor: pointer; padding: 0.5rem; background: #f0f0f0; border-radius: 4px;">
                            Diagnostic Information (click to expand)
                        </summary>
                        <div style="padding: 1rem; background: #f8f8f8; border-radius: 4px; margin-top: 0.5rem; font-size: 0.9rem;">
                            ${diagnosticInfo}
                        </div>
                    </details>
                    ...
                    <ul style="text-align: left; display: inline-block;">
                        ...
                        <li>Test CDN access: <a href="https://cdnjs.cloudflare.com/" target="_blank">cdnjs</a>, <a href="https://cdn.jsdelivr.net/" target="_blank">jsdelivr</a></li>
                    </ul>
                    <p style="margin-top: 1rem; color: #666;">Note: This project requires both Ace Editor (for code editing) and Skulpt (for running Python).</p>
                    <button class="btn btn-primary" onclick="location.reload()">Retry (Refresh Page)</button>
                    <button class="btn btn-secondary" onclick="window.retryWithLongerTimeout()" style="margin-left: 0.5rem;">Retry with Longer Timeout</button>
                    
                    <script>
                        window.retryWithLongerTimeout = async function() {
                            document.getElementById('loading-message').innerHTML = '<h2>Retrying with extended timeout...</h2><div class="spinner"></div><p id="loading-status">Please wait up to 30 seconds...</p>';
                    ...
                    <div style="margin-top: 2rem; padding: 1rem; background: #f0f0f0; border-radius: 8px;">
                        ...
                        <ol style="text-align: left;">
                            ...
                            <li>Try accessing this page from a different network (home instead of school)</li>
                            <li>Use a VPN if CDN access is blocked in your region</li>
                        </ol>
                        <p style="margin-top: 1rem;"><strong>For Teachers/IT:</strong> If this is on a school network, please whitelist these domains:</p>
            ...
            if (typeof ace === 'undefined') {
                console.warn('Ace Editor not loaded, but continuing...');
            }
    ...
    window.addEventListener('beforeunload', (e) => {
        ...
        if (appState.editor && typeof appState.editor.destroy === 'function') {
            appState.editor.destroy();
        }
    });
    
    // Make retry function globally available for error page
    window.retryWithLongerTimeout = async function() {
        document.getElementById('loading-message').innerHTML = '<h2>Retrying with extended timeout...</h2><div class="spinner"></div><p id="loading-status">Please wait up to 30 seconds...</p>';
        setTimeout(() => main(), 2000);
    };
    
    // Wait for scripts to load, with extended time for slower connections
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            // Show loading progress
            const loadingStatus = document.getElementById('loading-status');
            if (loadingStatus) {
                loadingStatus.textContent = 'Waiting for Python environment to load...';
            }
            setTimeout(main, 7000); // Give scripts 7 seconds to load
        });
    } else {
        setTimeout(main, 7000); // Give scripts 7 seconds to load
    }

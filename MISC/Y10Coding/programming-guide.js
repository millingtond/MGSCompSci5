...
// REPLACE this function in programming-guide.js
function loadGuideTopic(topicId) {
    ...
    // Add event listeners for copy buttons inside the newly created content
    setupCodeCopyButtons();
    
    // This new function will find all <pre> blocks and turn them into editors
    initializeGuideCodeSnippets(); // <<< THIS LINE IS ADDED
    
    // Highlight active topic in the sidebar
...
// ADD this new function to programming-guide.js
function initializeGuideCodeSnippets() {
    const codeExamples = document.querySelectorAll('.guide-topic .code-example pre');
    
    codeExamples.forEach((preElement) => {
        const code = preElement.textContent || preElement.innerText;

        // Create a container for the editor and its button
        const container = document.createElement('div');
        container.className = 'guide-snippet-container';

        // Create the div for the Ace editor instance
        const editorDiv = document.createElement('div');
        editorDiv.className = 'guide-code-snippet';
        // Auto-set height based on number of lines, with a minimum
        const lineCount = code.split('\n').length;
        editorDiv.style.height = `${Math.max(80, lineCount * 16)}px`;

        // Create the "Run in Playground" button
        const runButton = document.createElement('button');
        runButton.className = 'btn btn-primary btn-sm run-in-playground-btn';
        runButton.textContent = 'Run in Playground';
        runButton.onclick = () => {
            if(window.showPlaygroundModal) window.showPlaygroundModal();
            setTimeout(() => {
                if(window.playground && window.playground.playgroundEditor) {
                    window.playground.playgroundEditor.setValue(code.trim(), 1);
                    showToast("Example code loaded into the Playground!", "success");
                }
            }, 200);
        };
        
        // Replace the old <pre> element with our new container
        preElement.replaceWith(container);
        container.appendChild(editorDiv);
        container.appendChild(runButton);

        // Initialize a read-only Ace editor
        const editor = ace.edit(editorDiv);
        editor.setTheme("ace/theme/monokai"); // Or another theme you prefer
    ...
    // Remove the now-redundant "Open in Playground" buttons from the .practice-box divs
    document.querySelectorAll('.practice-box .btn-primary').forEach(btn => btn.remove());
}

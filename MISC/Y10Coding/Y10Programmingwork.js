...
(function() {
    // ADD this to the top of Y10Programmingwork.js
const BACKUP_SECRET_KEY = "gcse_python_project_!_super_secret_key_2025_#";

/**
 * Creates a SHA-256 signature for a string of data.
 * @param {string} dataString The data to sign.
 * @param {string} secret The secret key.
 * @returns {Promise<string>} A hex string of the signature.
 */
async function createSignature(dataString, secret) {
    if (!crypto.subtle) {
        // Fallback for non-secure contexts like http://
        console.warn("Crypto API not available. Signature will be a simple concatenation.");
        return dataString + secret;
    }
    const textToEncode = dataString + secret;
    const textData = new TextEncoder().encode(textToEncode);
    ...
    // New Ace Editor instantiation for initializeEditor()
/**
 * Initializes the main Ace editor for a task.
 * This function handles creating the editor, setting its options,
 * loading the appropriate code, and blocking paste functionality.
 *
 */
function initializeEditor(starterCode) {
    ...
    if (appState.editor) {
        ...
        if (appState.editor.container) {
            appState.editor.container.remove();
        }
    }

    // Create a new div for Ace to attach to
    const editorDiv = document.createElement('div');
    editorDiv.id = 'ace-editor'; // This ID can be reused as we destroy the old one
    editorContainer.appendChild(editorDiv);

    // Hide the original textarea if it's still there
    const oldTextarea = document.getElementById('editor');
    if(oldTextarea) oldTextarea.style.display = 'none';

    // Initialize Ace on the new div
    appState.editor = ace.edit(editorDiv);

    // Set editor options
    appState.editor.setTheme("ace/theme/monokai"); // Default theme
    appState.editor.session.setMode("ace/mode/python");
    appState.editor.setOptions({
        enableBasicAutocompletion: true,
        enableLiveAutocompletion: false, // Can be set to true if desired
        autoScrollEditorIntoView: true,
        copyWithEmptySelection: true,
        showPrintMargin: false,
    });
    appState.editor.renderer.setOption('showLineNumbers', true);
    appState.editor.session.setUseSoftTabs(true);
    appState.editor.session.setTabSize(4);

    // Set the initial code content
    const savedCode = appState.userProgress[appState.currentTaskId]?.code || starterCode;
    appState.editor.setValue(savedCode, -1); // -1 moves cursor to the start

    // --- PASTE-BLOCKING LOGIC ---
    // 1. Intercept Ace's internal paste command
    appState.editor.commands.on("exec", function(e) {
        if (e.command.name === "paste") {
            e.preventDefault(); // Stop the command
        }
    });

    // 2. Block the browser's native paste event as a fallback
    appState.editor.container.addEventListener('paste', function(e) {
        e.preventDefault();
    }, true);
    // --- END OF PASTE-BLOCKING ---

    // Set up the change listener to save progress
    ...
    if (window.appState.themeManager) {
        window.appState.themeManager.applyTheme(window.appState.themeManager.currentTheme);
    }
}
...
window.copyToClipboard = async function(text) {
    ...
    // REPLACE this function in Y10Programmingwork.js
async function exportFullProgress() {
    const dataToBackup = {
        version: '1.2-secure',
    ...
    // Create a signature from the most critical data
    const dataString = JSON.stringify({ xp: dataToBackup.xpData.totalXP, tasks: Object.keys(dataToBackup.progress).length });
    const signature = await createSignature(dataString, BACKUP_SECRET_KEY);

    const backupFileContent = {
        payload: dataToBackup,
        signature: signature
    };

    const blob = new Blob([JSON.stringify(backupFileContent, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    ...
    showToast('Secure progress backup downloaded!', 'success');
}
...
function importFullProgress() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (!backup.payload || !backup.signature) {
                throw new Error('Invalid backup file: Missing payload or signature.');
            }
            
            const dataToValidate = backup.payload;
            
            const dataString = JSON.stringify({ xp: dataToValidate.xpData.totalXP, tasks: Object.keys(dataToValidate.progress).length });
            const expectedSignature = await createSignature(dataString, BACKUP_SECRET_KEY);

            if (expectedSignature !== backup.signature) {
                showToast('Backup file is invalid or has been altered. Restore failed.', 'error');
                console.error("Signature mismatch! Restore aborted.");
                return;
            }

            if (confirm('Valid backup file detected. This will replace all your current progress. Are you sure?')) {
                appState.userProgress = dataToValidate.progress;
                appState.xpData = dataToValidate.xpData;
                appState.streakData = dataToValidate.streakData || { currentStreak: 0, longestStreak: 0, lastActiveDate: null, totalDaysActive: 0 };
                appState.completedSections = dataToValidate.completedSections || {};
            ...
            showToast('Failed to import backup. File may be corrupted or invalid.', 'error');

...
class InteractiveDebugger {
    constructor() {
        ...
        this.executionTrace = [];
        this.currentLineMarkerId = null; // <<< ADD THIS LINE HERE
    }
...
`;
    ...
    // We need a property to store the marker's ID so we can remove it later
    ...
    if (this.currentLineMarkerId) {
        editor.session.removeMarker(this.currentLineMarkerId);
    }

    // Add a new marker for the current line. lineNumber is 1-based, Ace is 0-based.
    this.currentLineMarkerId = editor.session.addMarker(
        new Range(lineNumber - 1, 0, lineNumber - 1, 1), 
        "current-line-indicator", // The CSS class for the marker
        "fullLine"
    );
}
...
applyTheme(theme) {
    ...
    // This function now ONLY controls the main page theme.
    // The editor theme is handled independently.
}
...
/**
 * Initializes the Ace editor for the Code Playground modal.
 * This function creates the editor, sets its options, loads saved code,
 * defines custom keybindings, and blocks paste functionality.
 *
 */
// REPLACE the entire CodePlayground class in enhanced-features.js
...
class CodePlayground {
    ...
    create() {
        ...
        editorHeader.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <button id="playground-theme-btn" aria-label="Toggle editor theme" class="btn btn-secondary btn-sm">🌙</button>
                <button onclick="window.playground.run()" style="background: #4CAF50; color: white; border: none; padding: 6px 16px; border-radius: 4px; cursor: pointer;">▶ Run</button>
                <button onclick="window.playground.clear()" style="background: #555; color: white; border: none; padding: 6px 16px; border-radius: 4px; cursor: pointer;">Clear</button>
                <button onclick="window.playground.save()" style="background: #2196F3; color: white; border: none; padding: 6px 16px; border-radius: 4px; cursor: pointer;">💾 Save</button>
            </div>
        `;
        ...
        editorDiv.id = 'playground-editor-div'; // Use a unique ID for the playground editor
        ...
        // ... (rest of the create() method from the source file for the output panel)
    ...
    // REPLACE the init() method in the CodePlayground class in enhanced-features.js

init() {
    const editorDiv = document.getElementById('playground-editor-div');
    if (!editorDiv) {
         console.error('Playground editor div not found');
         return;
    }
    
    // 1. Initialize Ace editor
    this.playgroundEditor = ace.edit(editorDiv);

    // --- START: NEW INITIALIZATION ORDER ---

    // 2. Set the content FIRST
    const saved = localStorage.getItem('pythonProjectPlayground');
    if (saved) {
        this.playgroundEditor.setValue(saved, -1);
    } else {
        this.playgroundEditor.setValue(`# Welcome to the Python Playground!\n# Write your code here and click Run.\n\nprint("Hello, Playground!")`, -1);
    }
    
    // 3. Now set mode and theme
    const aceTheme = window.appState.editorTheme === 'dark' ? 'ace/theme/monokai' : 'ace/theme/textmate';
    this.playgroundEditor.setTheme(aceTheme);
    this.playgroundEditor.session.setMode("ace/mode/python");

    // --- END: NEW INITIALIZATION ORDER ---

    // 4. Set other options and keybindings
    this.playgroundEditor.renderer.setOption('showLineNumbers', true);
    this.playgroundEditor.session.setUseSoftTabs(true);
    this.playgroundEditor.session.setTabSize(4);
    
    this.playgroundEditor.commands.addCommand({
        name: 'runCodePlayground',
        bindKey: {win: 'Ctrl-Enter', mac: 'Cmd-Enter|Ctrl-Enter'},
        exec: () => this.run()
    });
    
    // Paste-blocking logic
    this.playgroundEditor.commands.on("exec", function(e) {
        if (e.command.name === "paste") {
            e.preventDefault();
        }
    });
    this.playgroundEditor.container.addEventListener('paste', function(e) {
        e.preventDefault();
    }, true);

    // Button logic
    const themeBtn = document.getElementById('playground-theme-btn');
    if (themeBtn) {
        themeBtn.innerHTML = window.appState.editorTheme === 'dark' ? '☀️' : '🌙';
        themeBtn.addEventListener('click', () => {
            window.appState.editorTheme = window.appState.editorTheme === 'dark' ? 'light' : 'dark';
            const newAceTheme = window.appState.editorTheme === 'dark' ? 'ace/theme/monokai' : 'ace/theme/textmate';
            this.playgroundEditor.setTheme(newAceTheme);
            themeBtn.innerHTML = window.appState.editorTheme === 'dark' ? '☀️' : '🌙';
            if(window.saveEditorTheme) window.saveEditorTheme();
        });
    }
}
    
    async run() { /* ... function content from source file ... */ }
    clear() { /* ... function content from source file ... */ }
    save() { /* ... function content from source file ... */ }
}

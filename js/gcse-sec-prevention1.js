...
window.setupFirewallRuleLogic = function() {
    ...
    shuffledComponents.forEach(item => {
        ...
        div.addEventListener('dragstart', e => { draggedFirewallRuleItem = e.target; setTimeout(() => e.target.classList.add('opacity-50'), 0); });
        div.addEventListener('dragend', e => { setTimeout(() => e.target.classList.remove('opacity-50'), 0); });
    ...
    firewallRuleDescriptions.forEach(desc => {
        ...
        const dropTargetArea = dzDiv; // The whole div is the drop target
        ...
        dropTargetArea.addEventListener('drop', e => {
            ...
            if (draggedFirewallRuleItem) {
                ...
                draggedFirewallRuleItem.classList.remove('opacity-50');
            }
        });
...
window.checkFirewallRuleLogic = function() {
    ...
    dropzones.forEach(zone => {
        ...
        zone.classList.add('border-gray-300'); // Default border
        ...
        } else if (droppedItems.length > 0) { // Attempted but wrong number of items
            zone.classList.remove('border-gray-300');
            zone.classList.add('border-red-500');
        }
    });
...
window.checkDesignedFirewallRule = function() {
    ...
    const action = document.getElementById('firewall-rule-action').value;
    const protocolPort = document.getElementById('firewall-rule-protocol').value;
    if (feedbackDiv) {
        if(action && protocolPort) {
            ...
            feedbackDiv.innerHTML = '<span class="text-yellow-600">Please select an action and specify protocol/port for your rule.</span>';
        }
...
window.setupPenTestPhaseOrder = function() {
    ...
    bankContainer.innerHTML = '<h4 class="font-semibold text-gray-700 text-center mb-2">Phases (Drag these)</h4>';
    dropzonesContainer.innerHTML = '<h4 class="font-semibold text-gray-700 text-center mb-2">Correct Order (Drop & Describe)</h4>';
    feedbackEl.innerHTML = '';
    feedbackEl.classList.remove('show');
    ...
    shuffledPhases.forEach(phase => {
        ...
        div.addEventListener('dragstart', e => { draggedPenTestPhase = e.target; setTimeout(() => e.target.classList.add('opacity-50'), 0); });
        div.addEventListener('dragend', e => { setTimeout(() => e.target.classList.remove('opacity-50'), 0); });
        bankContainer.appendChild(div);
    });

    for (let i = 0; i < penTestPhasesData.length; i++) {
        const phaseData = penTestPhasesData[i]; // Use original order for dropzones
        const dzDiv = document.createElement('div');
        ...
        dropTargetArea.addEventListener('drop', e => {
            ...
            if (draggedPenTestPhase) {
                if (dropTargetArea.querySelector('.drag-item-phase')) { bankContainer.appendChild(dropTargetArea.querySelector('.drag-item-phase')); }
                dropTargetArea.innerHTML = ''; 
                dropTargetArea.appendChild(draggedPenTestPhase);
                draggedPenTestPhase.classList.remove('opacity-50');
            }
        });
        dropzonesContainer.appendChild(dzDiv);
    }
...
window.checkPenTestPhaseOrder = function() {
    ...
    dropzones.forEach((zone, index) => {
        ...
        if(dropTargetArea) dropTargetArea.classList.remove('border-green-500', 'border-red-500', 'border-gray-400'); 
        if(dropTargetArea) dropTargetArea.style.borderColor = ''; 

        if (droppedItem) {
            if (droppedItem.dataset.phaseId === penTestPhasesData[index].id) {
                correctOrderCount++;
                if(dropTargetArea) dropTargetArea.style.borderColor = '#10b981'; 
            } else {
                if(dropTargetArea) dropTargetArea.style.borderColor = '#ef4444'; 
            }
        } else {
...
window.selectPenTestTypeOption = function(buttonEl, selectedType, index) {
    ...
    itemContainer.querySelectorAll('.activity-option').forEach(btn => btn.classList.remove('selected'));
...
window.checkPenTestTypeChoices = function() {
     penTestTypeScenariosData.forEach((item, index) => {
        ...
        const selectedButton = itemContainer ? itemContainer.querySelector('.activity-option.selected') : null;
        const userAnswer = itemContainer ? itemContainer.dataset.selectedType : null;

        if (selectedButton && userAnswer) {
            if (userAnswer === item.correctAnswer) {
                selectedButton.classList.add('correct');
                feedbackEl.innerHTML = `<span class="text-green-600 font-semibold"><i class="fas fa-check-circle mr-1"></i>Correct!</span> ${item.explanation}`;
            } else {
                selectedButton.classList.add('incorrect');
                feedbackEl.innerHTML = `<span class="text-red-600 font-semibold"><i class="fas fa-times-circle mr-1"></i>Not quite.</span> The best choice is ${item.correctAnswer}. ${item.explanation}`;
            }
        } else if(feedbackEl) {
...
window.checkFillBlanks = window.checkFillBlanks || function(containerId) { /* Basic placeholder */ console.warn("checkFillBlanks not fully defined locally"); };
window.resetFillBlanks = window.resetFillBlanks || function(containerId) { /* Basic placeholder */ console.warn("resetFillBlanks not fully defined locally"); };
